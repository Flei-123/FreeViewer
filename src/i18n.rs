...
const TABLE: &[(&str, &str, &str)] = &[
    ...
    ("dep.title", "Neues Geraet per Link einrichten", "Set up a new device by link"),
    ("dep.lead", "Einmal-Link erzeugen, auf dem anderen Geraet oeffnen, Namen vergeben - FreeViewer installiert sich dort (mit Dienst) und das Geraet taucht hier mit ID und Passwort auf.", "Create a one-time link, open it on the other device, pick a name - FreeViewer installs itself there (with service) and the device shows up here with ID and password."),
    ("dep.need_login", "Dafuer musst du mit deinem Konto angemeldet sein.", "You need to be signed in for this."),
    ("dep.to_account", "Zum Konto", "Go to account"),
    ("dep.pw", "Passwort fuer das neue Geraet", "Password for the new device"),
    ("dep.pw_new", "Neu wuerfeln", "Roll again"),
    ("dep.name_hint", "Namens-Vorschlag (kann dort geaendert werden)", "Name suggestion (can be changed there)"),
    ("dep.name_hint_ph", "z. B. Wohnzimmer-PC", "e.g. living-room PC"),
    ("dep.make", "Link erzeugen", "Create link"),
    ("dep.making", "Erzeuge …", "Creating …"),
    ("dep.link", "Diesen Link auf dem neuen Geraet oeffnen:", "Open this link on the new device:"),
    ("dep.copy", "In die Zwischenablage", "Copy to clipboard"),
    ("dep.link_note", "Hat das Geraet schon FreeViewer, oeffnet sich das Einrichtungs-Fenster. Sonst laedt der Link das kleine Setup herunter, das nach dem Code fragt.", "With FreeViewer present the setup window opens. Without, the link downloads the small setup which asks for the code."),
    ...
    ("dep.waiting", "wartet auf das andere Geraet", "waiting for the other device"),
    ("dep.done_row", "eingerichtet - im Adressbuch", "set up - in the address book"),
    ("setup.title", "Dieses Geraet einrichten", "Set up this device"),
    ("setup.lead", "Ueber diesen Einmal-Link wird FreeViewer hier richtig installiert (nach Programme, mit Dienst - also auch vor der Anmeldung erreichbar). Das Geraet taucht danach in der Liste des Absenders auf, fertig zum Verbinden.", "This one-time link installs FreeViewer properly (Program Files, with service - reachable before sign-in). The device then shows up in the sender's list, ready to connect."),
    ("setup.name", "Wie soll dieses Geraet heissen?", "What should this device be called?"),
    ("setup.ok", "Einrichten und installieren", "Set up and install"),
    ("setup.cancel", "Abbrechen", "Cancel"),
    ("setup.working", "Richte ein …", "Setting up …"),
    ("setup.done", "Fertig - die Installation laeuft. {} sieht dieses Geraet jetzt in seiner Liste.", "Done - installation is running. {} now sees this device in the list."),
