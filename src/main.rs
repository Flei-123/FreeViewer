...
fn main() -> eframe::Result<()> {
    ...
    // Empfangstest (Stufe 2c): Bilder der anderen dekodieren und messen.
    ...
    if let Some(i) = std::env::args().position(|a| a == "--meetempfang") {
        ...
        let ton = match meetrtc::starten() {
            Ok(t) => t,
            Err(e) => {
                println!("EMPFANG FEHLER Ton: {}", e);
                return Ok(());
            }
        };
        let mut dek = meetvideo::Dekodierer::neu();
        let mut angeboten = false;
        let mut rahmen: u64 = 0;
        ...
        while start.elapsed().as_secs() < dauer {
            ...
            for te in ton.abholen() {
                if let meetrtc::TonEreignis::Bild { quelle, daten, .. } = te {
                    rahmen += 1;
                    dek.rahmen(quelle, &daten);
                }
            }
            std::thread::sleep(std::time::Duration::from_millis(5));
        }
        let mut zeile = String::new();
        for (peer, (w, h, _)) in dek.bilder.iter() {
            zeile.push_str(&format!(
                " peer={} bild={}x{} helligkeit={:.3}",
                peer,
                w,
                h,
                dek.helligkeit(*peer)
        ...
        println!(
            "EMPFANG rahmen={} dekodiert={} bilder={}{} fehler={}",
            rahmen,
            dek.gezaehlt,
            dek.bilder.len(),
            zeile,
